...
export class TierListService {
  ...
  async findOne(id: string, userId: string): Promise<TierList> {
    ...
    if (tierList.user.userId !== userId) {
      throw new ForbiddenException('You do not have permission to access this tier list');
    }
    return tierList;
  }

  async update(id: string, updateTierListDto: UpdateTierListDto, userId: string): Promise<TierList> {
